...
createContainers(){
  ...
  id_max=$((id_already + ${CONTAINER_NUMBER}))
	...
	for i in $(seq $id_min $id_max);do
		sudo podman run -d --systemd=true --publish-all=true -v /srv/data:/srv/data --name ${CONTAINER_USER}-debian-$i -h ${CONTAINER_USER}-debian-$i docker.io/bcastets/ansible-dev:10
